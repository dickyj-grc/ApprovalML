...
## Mock Data for Automatic Steps (Optional)

`mock_data` is a step-level key on `data_processor` steps that bypasses the real connector in **test mode**. Instead of fetching from the external API, the engine uses the stored JSON directly, applies `field_mapping` on top of it, and advances the step as APPROVED. Production mode ignores `mock_data` completely.

This allows a full test-mode workflow run without needing live connector credentials or external API availability.
...
get_customer_data:
  ...
  data_processor:
    ...
    params:
      - name: partner_id
        from_field: field.customer_id
  field_mapping:
    customer_phone: "$.data[0].phone"
    customer_email:
      source: "$.data[0].email"
      jsonata: "$lowercase(value)"
  mock_data:                          # used in test mode instead of real fetch
    data:
      - phone: "+1 555 123 4567"
        email: "JOHN@EXAMPLE.COM"
        name: "John Doe"
  on_complete:
    continue_to: send_welcome_email
```

**Rules:**
- `mock_data` must be a YAML mapping (dict). Its shape must match what the real connector returns so `field_mapping` JSONPath expressions evaluate correctly against it.
- The value is typically generated by the Workflow Studio's **Field Mapping Builder** dialog — run a live test, build the mapping, then click "Apply + save mock_data" to write both `field_mapping` and `mock_data` into the YAML in one step.
- `mock_data` is silently ignored in production; it is only active when `instance.metadata.is_test_mode == true`.
- If `save_to` is configured on the `data_processor` block, the mock response is also saved to that workflow variable so downstream steps can reference it via `vars.<save_to>`.
